...
async function handleSubmit() {
	const submitObject = {
		...
		context: `來源：${dialogStore.issue.id} - ${dialogStore.issue.index} - ${dialogStore.issue.name}`,
		status: "待處理",
	};
...
function handleClose() {
	...
	dialogStore.dialogs.reportStopPower = false;
}
</script>

<template>
  <DialogContainer
    dialog="reportStopPower"
    @on-close="handleClose"
  >
    <div class="reportissue">
      <h2>停電回報</h2>
      <h3>地址* ({{ allInputs.title.length }}/100)</h3>
      <input
